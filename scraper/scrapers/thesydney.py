...
class TheSydneyScraper(BaseScraper):
    name = "The Sydney"
    id = "thesydney"
    url = "https://thesydneybenson.com/events/"
    ...
    def parse_events(self, html: str) -> list[Event]:
        ...
        # Event cards are in div.eventWrapper.rhpSingleEvent (same as Waiting Room)
        for card in soup.select("div.eventWrapper.rhpSingleEvent"):
            try:
                # Title - h2 with class containing rhp-event__title
                title_el = card.select_one('h2[class*="rhp-event__title"], [class*="rhp-event__title--list"]')
                title = title_el.get_text(strip=True) if title_el else None
                if not title:
                    continue

                # Date - div.eventMonth.singleEventDate (format: "Fri, Feb 27")
                ...
                event_info_el = card.select_one('.eventMoreInfo a, a[href*="thesydneybenson.com/events/"]')
                ...
                # Supporting Artists - look for "with Artist, Artist" pattern
                supporting_artists = None

                # Check subheader and h4 tags
                for el in card.select(".eventSubHeader, h4"):
                    el_text = el.get_text(strip=True)
                    if el_text.lower().startswith("with "):
                        artists_str = el_text[5:]  # Remove "with "
                ...
                time_el = card.select_one('.rhp-event__time-text--list, [class*="time-text"]')
                time_str = self._parse_time(time_el.get_text(strip=True)) if time_el else None

                # Generate ID
                slug = re.sub(r'[^a-z0-9]+', '-', title.lower()).strip('-')
                event_id = f"thesydney-{date_str}-{slug}"[:80]
    ...
    def _parse_date(self, date_text: str) -> str | None:
        """Convert date text like 'Fri, Feb 27' to YYYY-MM-DD format."""
        try:
            date_text = date_text.strip()

            # Handle format "Fri, Feb 27" or "Sat, Mar 06"
            if ',' in date_text:
    ...
    def _parse_time(self, time_text: str) -> str | None:
        """Convert time text like 'Show: 7:30 pm' to HH:MM format (24-hour)."""
        try:
