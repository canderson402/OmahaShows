#!/usr/bin/env python3
"""
Run the Ticketmaster API client - creates pending events for admin review.
Fetches music events from Omaha/Lincoln metro area.
Skips events that already exist in the database.

Usage:
    python run_ticketmaster.py

Or via GitHub Actions with SCRAPER_ID=ticketmaster
"""
import os
import re
import sys
...
if not TICKETMASTER_API_KEY:
    print("ERROR: Missing TICKETMASTER_API_KEY environment variable")
    sys.exit(1)
...
def run():
    print(f"\n{'='*60}")
    print(f"TICKETMASTER SCRAPE - {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
    print(f"{'='*60}\n")
    ...
    try:
        print("Fetching from Ticketmaster API...")
        ...
        for event in future_events:
            ...
            # source is already set to matched venue_id by the client
            ...
            venue_display = event.venue if venue_id == "other" else venue_id
            print(f"  + {event.title} ({event.date}) @ {venue_display}")

        # Log the scraper run
        supabase.table("scraper_runs").insert({
            "scraper_id": "ticketmaster",
            "scraper_name": "Ticketmaster",
            "status": "success",
        ...
        if new_ids:
            ...
            for eid in new_ids[:10]:  # Show first 10
                print(f"  - {eid}")
            if len(new_ids) > 10:
